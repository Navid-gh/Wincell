...
const Courses = () => {
    ...
    const { data, isLoading, isError, error } = useQuery({
        queryKey: ['courses'],
        queryFn: () => getCourses(undefined, undefined, undefined, undefined),
    });
            ...
            <div className='flex flex-col gap-12'>
                ...
                <ul className='flex flex-col gap-6'>
                    {data?.map(item => {
                        ...
                        return (
                            <li key={item._id} className='flex flex-col border-b-2 border-black'>
                                ...
                                <Link to={`Edit`} className='text-yellow max-w-fit' state={item}>
                                    <span>تغییر دوره</span>
                                </Link>
                                <Link to={'comments'} className='text-yellow max-w-fit' state={{ id: item._id, comments: item.comments }}>
                                    <span>مدیریت کامنت ها</span>
                                </Link>
                                <Link to={'chapters'} className='text-yellow max-w-fit' state={{ id: item._id, chapters: item.chapters }}>
