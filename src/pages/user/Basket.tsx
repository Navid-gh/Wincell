...
const Basket = () => {
  ...
  const handleBasket = async (courseId: string) => {
    try {
      const updatedCourseIds = basketData.productsId.filter(
        (id) => id !== courseId
      );
  ...
  const totalPrice =
    data?.listCourse.reduce(
      (total: number, course: Course) => total + course.price,
      0
    ) || 0;
  const totalDiscount =
    data?.listCourse.reduce(
      (total: number, course: Course) =>
        total + (course.discount + discountResult || 0),
      0
    ) || 0;
  const totalPriceWithDiscount =
    data?.listCourse.reduce(
      (total: number, course: Course) =>
        total + (course.price - (course.discount + discountResult || 0)),
      0
    ) || 0;
    ...
    <div className="flex flex-col gap-6">
      ...
      <WithLoaderAndError {...{ data, isError, isLoading, error }}>
        <div className="flex items-start gap-4">
          ...
          <div className="flex flex-col gap-4 py-4 bg-main-secondary-bg min-w-[22rem] rounded-small">
            <div className={cn("px-5 text-main-primary-text", textBody1Bold)}>
              <h2>جزئیات خرید</h2>
            </div>
            ...
            <div className="px-4">
              <Button
                intent="primary"
                size="base"
                className="py-4 border border-main-primary-text"
              >
                خرید دوره
              </Button>
            </div>
